    ...
    try {
      await setDoc(
        doc(db, "enrollments", row.id),
        {
          status: nextStatus,
          checkedInAt:
            nextStatus === "active" ? serverTimestamp() : null,
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        label="Admin"
        title="Enrollments"
        description="Activate or deactivate user access."
      />
      ...
      <FlatTable headers={["User", "Status", "Checked In", ""]}>
        {filtered.map((enrollment) => (
          <FlatRow key={enrollment.id}>
            ...
            <FlatCell>
              {formatTimestamp(enrollment.checkedInAt)}
            </FlatCell>
            ...
            <FlatCell className="text-right">
              {enrollment.status === "active" ? (
                <Button
                  ...
                  onClick={() =>
                    updateStatus(enrollment, "not_enrolled")
                  }
                >
                  Deactivate
                </Button>
                ...
                <Button
                  ...
                  disabled={busyId === enrollment.id}
                  onClick={() =>
                    updateStatus(enrollment, "active")
                  }
                >
                  Activate
                </Button>
